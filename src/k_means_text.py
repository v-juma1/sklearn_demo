...
"""
def text_cluster():
    docs=load_files(r"data\clustering\data")
    ...
    vector=TfidfVectorizer(max_df=0.4,min_df=2,max_features=20000,encoding="latin-1")
    ...
    print(metrics.silhouette_score(x,kmean.labels_,sample_size=1000))#0.0052792296749458855
...
def cluster_evaluate():
    ...
    print(metrics.adjusted_rand_score(true_label,pred_label))#-0.17647058823529416
    ...
    print(metrics.homogeneity_score(true_label,pred_label))#0.9999999999999999
    ...
    pred_label=np.random.randint(1,4,6)
    #对随机序列，齐次性值不为0
    print(metrics.homogeneity_score(true_label,pred_label))#0.20751874963942177
    ...
    true_label=[0,1,2,3]
    pred_label=[1,1,2,2]
    print(metrics.completeness_score(true_label,pred_label))#0.9999999999999999
    ...
    true_label=np.random.randint(1,4,6)
    pred_label=np.random.randint(1,4,6)
    print(metrics.completeness_score(true_label,pred_label))#0.41102631318192145
