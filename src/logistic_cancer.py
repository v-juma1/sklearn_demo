...
for i in range(len(degree)):
    ...
    model=polynomail_model(degree=degree[i],penalty=penalty,solver='liblinear',max_iter=300)
    plot_learning_curve(plt,model,title.format(degree[i],penalty),x,y,ylim=(0.8,1.01),cv=cv)
plt.show()
...
for i in range(len(degree)):
    ...
    model=polynomail_model(degree=degree[i],penalty=penalty,solver='lbfgs',max_iter=1000)
    plot_learning_curve(plt,model,title.format(degree[i],penalty),x,y,ylim=(0.8,1.01),cv=cv)
plt.show()
