...
for i in range(len(degree)):
    ...
    model = polynomail_model(degree=degree[i],
                             penalty=penalty,
                             solver='liblinear',
                             max_iter=300)
    plot_learning_curve(plt,
                        model,
                        title.format(degree[i], penalty),
                        x,
                        y,
                        ylim=(0.8, 1.01),
                        cv=cv)
plt.savefig("scikit-learn/log/logistic_l1_curve.png")
...
for i in range(len(degree)):
    ...
    plot_learning_curve(plt,
                        ...
                        ylim=(0.8, 1.01),
                        cv=cv)
plt.savefig("scikit-learn/log/logistic_l2_curve.png")
