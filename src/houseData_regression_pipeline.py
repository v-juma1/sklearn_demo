...
def fine_tuning(model, param, datax, datay):
    ...
    neg_score = cross_val_score(grid_search.best_estimator_,
                                datax,
                                datay,
                                scoring="neg_mean_squared_error",
                                cv=5,
...
def run(model_name, task):
    # 加载数据
    data = load_house_data("scikit-learn\data\housing\housing.csv")
    ...
    if model_name == "LinearRegression":
        ...
        pre_model = Pipeline([("pf", polynomial_features),
        ...
        model_path = "scikit-learn/model/house_LinearRegression.pkl"

    model = house_model(model_path)

    if task == "train":
        best_params = fine_tuning(pre_model, grid_params, clean_train_data,
                                  train_label)
